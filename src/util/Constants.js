...
const Endpoints = exports.Endpoints = {
  User: userID => {
    ...
    return {
      ...
      note: id => `${base}/notes/${id}`,
      billing: {
        paymentSources: `${base}/billing/payment-sources`,
        subscriptions: `${base}/billing/subscriptions`,
      },
      virtualCurrency: `${base}/virtual-currency/balance`,
  ...
  Guild: guildID => {
    ...
    return {
      ...
      widget: `${base}/widget.json`,
      onboarding: `${base}/onboarding`,
      onboardingResponses: `${base}/onboarding-responses`,
      memberVerification: `${base}/member-verification`,
      memberRequests: `${base}/requests/@me`,
    };
  },
  channels: '/channels',
  Channel: channelID => {
  ...
  App: appID => ({
    externalAssets: `/applications/${appID}/external-assets`,
  }),
  auth: {
    login: '/auth/login',
    logout: '/auth/logout',
    sessions: '/auth/sessions',
    sessionsLogout: '/auth/sessions/logout',
    mfaTotp: '/auth/mfa/totp',
    remoteAuthLogin: '/users/@me/remote-auth/login',
  },
  login: '/auth/login',
  logout: '/auth/logout',
  stickers: id => `/stickers/${id}`,
  stickerPacks: '/sticker-packs',
  quests: id => (id ? `/quests/${id}` : '/quests/@me'),
  attachments: {
    refresh: '/attachments/refresh-urls',
  },
  entitlements: {
    redeem: code => `/entitlements/gift-codes/${code}/redeem`,
  },
  oauth2: {
    authorize: '/oauth2/authorize',
    tokens: id => (id ? `/oauth2/tokens/${id}` : '/oauth2/tokens'),
  },
  applications: id => (id ? `/applications/${id}` : '/applications'),
  applicationsPublic: ids => `/applications/public?${ids.map(i => `application_ids=${i}`).join('&')}`,
...
/**
 * Relationship types for user accounts
