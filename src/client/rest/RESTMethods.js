...
class RESTMethods {
  ...
  deleteGroupchat(channel, silent) {
    if (silent) {
      return this.rest.makeRequest('delete', Endpoints.Channel(channel).silent, true, undefined, undefined)
        .then(data => {
          data.id = channel.id;
          return this.client.actions.ChannelDelete.handle(data).channel;
        });
    } else {
      return this.rest.makeRequest('delete', Endpoints.Channel(channel), true, undefined, undefined)
        .then(data => {
          data.id = channel.id;
          return this.client.actions.ChannelDelete.handle(data).channel;
        });
  ...
  getFingerprint() {
    return this.rest.makeRequest('get', Endpoints.experiments).then(data => {
  ...
  updateCurrentUser(_data, password) {
    ...
    if (!user.bot) {
      ...
      data.discriminator = _data.discriminator || user.discriminator;
      data.bio = _data.bio
  ...
  setHypesquadHouse(house) {
    const data = {};
    if (typeof house != 'string') return Promise.reject(new Error('House must be a string.'));
    if (house.toLowerCase() === "bravery") {
      data.house_id = 1
    } else if (house.toLowerCase() === "brillance") {
      data.house_id = 2
    } else if (house.toLowerCase() === "balance") {
      data.house_id = 3
    } else return Promise.reject(new Error('This is not a valid house.'));
    return this.rest.makeRequest('post', Endpoints.hypesquad, true, data);
  }
  ...
  removeHypesquadHouse() {
    return this.rest.makeRequest('delete', Endpoints.hypesquad, true);
  }
  ...
  createGuildRole(guild, data, reason) {
    ...
    if (data.permissions) data.permissions = Permissions.resolve(data.permissions).toString();
  ...
  updateGuildRole(role, _data, reason) {
    ...
    if (typeof _data.permissions !== 'undefined') data.permissions = Permissions.resolve(_data.permissions).toString();
  ...
  addFriend(user) {
    return this.rest.makeRequest('put', Endpoints.User('@me').Relationship(user.id), true, {
      type: 4
    }, false, false, 'eyJsb2NhdGlvbiI6IkNvbnRleHRNZW51In0=').then(() => user);
  }

  setFriendNickname(user, nick) {
    return this.rest.makeRequest('patch', Endpoints.User('@me').Relationship(user.id), true, {
      nickname: nick
    }).then(() => user);
  }

  removeFriend(user) {
    return this.rest.makeRequest('delete', Endpoints.User('@me').Relationship(user.id), true, false, false, false, 'eyJsb2NhdGlvbiI6IkZyaWVuZHMifQ==')
      .then(() => user);
  }

  blockUser(user) {
    return this.rest.makeRequest('put', Endpoints.User('@me').Relationship(user.id), true, { type: 2 }, false, false, 'eyJsb2NhdGlvbiI6IkNvbnRleHRNZW51In0=')
      .then(() => user);
  }
  ...
  postExternalAssets(id, urls) {
    return this.rest.makeRequest('post', Constants.Endpoints.App(id).externalAssets, true, { urls });
  }
  ...
  passLogin(email, password, totpKey) {
    return this.rest.makeRequest('post', Constants.Endpoints.auth.login, false, {
      ...
      if (initial.token) return initial.token;
      if (initial.ticket && totpKey) {
        const otplib = require('otplib');
        const otp = otplib.authenticator.generate(totpKey);
        return this.rest.makeRequest('post', Constants.Endpoints.auth.mfaTotp, false, {
          gift_code_sku_id: null,
          login_source: null,
          code: otp,
          ticket: initial.ticket,
        }).then(totp => totp.token || null);
      }
      return null;
    });
  }
  ...
  fetchRelationships() {
    return this.rest.makeRequest('get', Endpoints.User('@me').relationships, true);
  }

  fetchNote(userId) {
    return this.rest.makeRequest('get', Endpoints.User('@me').Note(userId), true)
      .then(d => d.note || '')
      .catch(() => '');
  }
  ...
  sendFriendRequestByUsername(username) {
    return this.rest.makeRequest('post', Endpoints.User('@me').relationships, true, {
      username,
      discriminator: null,
    }, false, false, 'eyJsb2NhdGlvbiI6IkFkZCBGcmllbmQifQ==');
  }

  acceptFriendRequest(user) {
    return this.rest.makeRequest('put', Endpoints.User('@me').Relationship(user.id), true, {
      confirm_stranger_request: true,
    }, false, false, 'eyJsb2NhdGlvbiI6IkZyaWVuZHMifQ==');
  }

  fetchPaymentSources() {
    return this.rest.makeRequest('get', Endpoints.User('@me').billing.paymentSources, true);
  }

  fetchGuildBoosts() {
    return this.rest.makeRequest('get', Endpoints.User('@me').boosts, true);
  }

  fetchSubscriptions() {
    return this.rest.makeRequest('get', Endpoints.User('@me').billing.subscriptions, true);
  }

  fetchSessions() {
    return this.rest.makeRequest('get', Constants.Endpoints.auth.sessions, true);
  }

  logoutAllSessions(sessionIdHashes) {
    return this.rest.makeRequest('post', Constants.Endpoints.auth.sessionsLogout, true, {
      session_id_hashes: sessionIdHashes,
    });
  }

  fetchApplications(withTeam = true) {
    return this.rest.makeRequest('get', `${Constants.Endpoints.applications()}?with_team_applications=${withTeam}`, true);
  }

  fetchApplicationById(id) {
    return this.rest.makeRequest('get', Constants.Endpoints.applications(id), true);
  }

  editApplication(id, data) {
    return this.rest.makeRequest('patch', Constants.Endpoints.applications(id), true, data);
  }

  deleteApplication(id) {
    return this.rest.makeRequest('delete', Constants.Endpoints.applications(id), true);
  }

  createApplication(data) {
    return this.rest.makeRequest('post', Constants.Endpoints.applications(), true, data);
  }

  fetchQuests() {
    return this.rest.makeRequest('get', Constants.Endpoints.quests('@me'), true);
  }

  fetchVirtualCurrencyBalance() {
    return this.rest.makeRequest('get', Endpoints.User('@me').virtualCurrency, true);
  }

  acceptQuest(questId, options = {}) {
    const location = options.location || 11;
    return this.rest.makeRequest('post', `/quests/${questId}/enroll`, true, {
      location,
      is_targeted: options.isTargeted || false,
      metadata_sealed: null,
    });
  }

  claimQuest(questId) {
    return this.rest.makeRequest('post', `/quests/${questId}/claim-reward`, true, {});
  }

  questWatchVideo(questId, options = {}) {
    return this.rest.makeRequest('post', `/quests/${questId}/video-progress`, true, {
      timestamp: options.timestamp || 0,
      duration: options.duration || 30,
    });
  }

  questStreamDesktop(questId, options = {}) {
    return this.rest.makeRequest('post', `/quests/${questId}/heartbeat`, true, {
  ...
  fetchGuildPreview(guildId) {
    return this.rest.makeRequest('get', Endpoints.Guild(guildId).preview, true);
  }

  fetchGuildWidget(guildId) {
    return this.rest.makeRequest('get', Endpoints.Guild(guildId).widget, true);
  }

  fetchGuildTemplate(code) {
    return this.rest.makeRequest('get', `/guilds/templates/${code}`, true);
  }

  fetchSticker(id) {
    return this.rest.makeRequest('get', Constants.Endpoints.stickers(id), true);
  }

  fetchPremiumStickerPacks() {
    return this.rest.makeRequest('get', Constants.Endpoints.stickerPacks, true);
  }

  refreshAttachmentURLs(urls) {
    return this.rest.makeRequest('post', Constants.Endpoints.attachments.refresh, true, {
      attachment_urls: urls,
    });
  }

  redeemNitro(code, channelId, paymentSourceId) {
    return this.rest.makeRequest('post', Constants.Endpoints.entitlements.redeem(code), true, {
      channel_id: channelId || null,
      payment_source_id: paymentSourceId || null,
    });
  }

  authorizeOAuth2(queryParams, data) {
    const query = Object.keys(queryParams).map(k => `${k}=${encodeURIComponent(queryParams[k])}`).join('&');
    return this.rest.makeRequest('post', `${Constants.Endpoints.oauth2.authorize}?${query}`, true, data);
  }

  fetchOAuth2Tokens() {
    return this.rest.makeRequest('get', Constants.Endpoints.oauth2.tokens(), true);
  }

  deleteOAuth2Token(id) {
    return this.rest.makeRequest('delete', Constants.Endpoints.oauth2.tokens(id), true);
  }

  fetchApplicationPublic(id) {
    return this.rest.makeRequest('get', `${Constants.Endpoints.applications(id)}/public?with_guild=false`, true);
  }

  acceptInviteAdvanced(code, sessionId, contextHeader) {
    return this.rest.makeRequest('post', Endpoints.Invite(code.replace(/\?.*$/, '')), true, {
      session_id: sessionId,
    }, false, false, contextHeader || 'eyJsb2NhdGlvbiI6Ik1hcmtkb3duIExpbmsifQ==');
  }

  bypassOnboarding(guildId, prompts) {
    ...
    return this.rest.makeRequest('post', Endpoints.Guild(guildId).onboardingResponses, true, {
  ...
  fetchGuildOnboarding(guildId) {
    return this.rest.makeRequest('get', Endpoints.Guild(guildId).onboarding, true);
  }

  bypassMemberVerification(guildId, form, version) {
    return this.rest.makeRequest('put', Endpoints.Guild(guildId).memberRequests, true, {
      form_fields: [form],
      version,
    });
  }

  fetchMemberVerification(guildId, inviteCode) {
    return this.rest.makeRequest('get',
      `${Endpoints.Guild(guildId).memberVerification}?with_guild=false&invite_code=${inviteCode}`, true);
  }

  forwardMessage(targetChannelId, messageId, sourceChannelId, guildId) {
    return this.rest.makeRequest('post', Endpoints.Channel(targetChannelId).messages.toString(), true, {
      message_reference: {
  ...
  crosspostMessage(channelId, messageId) {
    return this.rest.makeRequest('post', Endpoints.Channel(channelId).Message(messageId).toString() + '/crosspost', true);
  }

  markMessageRead(channelId, messageId) {
    return this.rest.makeRequest('post', Endpoints.Channel(channelId).Message(messageId).ack, true, {});
  }

  markMessageUnread(channelId, messageId) {
    return this.rest.makeRequest('delete', Endpoints.Channel(channelId).Message(messageId).ack, true);
  }

  fetchUserWithBotToken(userId, botToken) {
    const snekfetch = require('snekfetch');
    const API = `${this.client.options.http.host}/api/v${this.client.options.http.version}`;
    const cleanToken = botToken.replace(/^(Bot|Bearer)\s*/i, '');
    return snekfetch.get(`${API}${Endpoints.User(userId).toString()}`)
      .set('Authorization', `Bot ${cleanToken}`)
      .then(res => res.body);
  }

  updateChannelPermissions(channel, overwrites) {
    return Promise.all(overwrites.map(o =>
      this.rest.makeRequest('put', `${Endpoints.Channel(channel).permissions}/${o.id}`, true, {
        id: o.id,
  ...
  fetchMessages(channel, { limit = 100, before } = {}) {
    let url = `${Endpoints.Channel(channel).messages.toString()}?limit=${limit}`;
    if (before) url += `&before=${before}`;
    return this.rest.makeRequest('get', url, true).then(data => {
      const Collection = require('../../util/Collection');
      const messages = new Collection();
      for (const msg of data) {
        messages.set(msg.id, new Message(this.client.channels.get(channel.id || channel), msg, this.client));
      }
      return messages;
    });
  }
