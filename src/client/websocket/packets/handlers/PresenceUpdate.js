...
class PresenceUpdateHandler extends AbstractHandler {
  handle(packet) {
    ...
    const friend = client.user.friends.get(data.user.id);

    if (friend && data.user.username) {
      const oldPresence = Util.cloneObject(friend);
      if (friend.presence) {
        oldPresence.frozenPresence = Util.cloneObject(friend.presence);
      }
      client._setPresence(data.user.id, data);
      client.emit(Constants.Events.FRIEND_PRESENCE_UPDATE, oldPresence, friend);
    }
    ...
    //let event = false;
    for (const guild of client.guilds.map(g => g)) {
