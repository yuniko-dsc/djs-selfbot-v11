...
class User {
  ...
  setup(data) {
    ...
    /**
     * The Display of the user
     * @type {string}
     */
    this.displayName = this.globalName ?? this.username;
   ...
   */
  get presence() {
    if (this.frozenPresence) return this.frozenPresence;
   ...
   */
  get tag() {
    const tag = this.discriminator === '0' ? `${this.username}#0000` : `${this.username}#${this.discriminator}`;
    return `${tag}`;
  }
   ...
   */
  get note() {
    if (this.client.notes) return this.client.notes.cache.get(this.id) || null;
    if (this.client.user && this.client.user.notes) return this.client.user.notes.get(this.id) || null;
    return null;
  }
  ...
  setFriendNickname(nick) {
    if (!this.client.user.friends.has(this.id)) throw new Error('You are not friend with this user');
    return this.client.rest.methods.setFriendNickname(this, nick);
  }
  /**
  ...
  /**
   * Sends a friend request (v13 alias).
   * @returns {Promise<User>}
   */
  sendFriendRequest() {
    return this.client.relationships.sendFriendRequest(this).then(() => this);
  }

  /**
   * Removes a relationship with this user.
  ...
  /**
   * Re-fetch this user from the API.
   * @returns {Promise<User>}
   */
  fetch() {
    return this.client.fetchUser(this.id).then(user => {
  ...
  /**
   * Get guild-scoped profile for this user.
   * @param {Snowflake} [guildId] Guild ID
   * @returns {Promise<UserProfile>}
   */
  getProfile(guildId) {
    return this.client.rest.methods.fetchUserProfile(this).then(profile => {
