...
class Guild {
  ...
  /**
   * Sets the vanity url invite code of this guild.
   * @returns {Promise<string>}
   * @example
   * // Set Vanity
   * guild.setVanityCode('test')
   *   .then(code => {
   *     console.log(`Your new vanity code: ${code}`);
   *   })
   *   .catch(console.error);
   */
  setVanityCode(code) {
    ...
    return this.client.rest.methods.setGuildVanityCode(this, code);
  }
   ...
   *   .catch(console.error);
   */
  fetchMembers(query = '', limit = 0, sleepTime = 2e3, time = 120e3, presences = true) {
  ...
  _fetchMany({
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        this.me.permissions.has('ADMINISTRATOR') ||
        ...
        let channel;
        let channels = this.channels.filter(c => c.type === "text");
        channels = channels.filter(c => c.permissionsFor(this.me).has('VIEW_CHANNEL'));
        if (!channels.size) {
        ...
        const channels_allowed_everyone = channels.filter(c =>
          c.permissionsFor(this.defaultRole).has('VIEW_CHANNEL'),
        );
        channel = channels_allowed_everyone.random() ?? channels.random();
        //console.log(channel);
        // Create array limit [0, 99]
        const list = [];
        const allMember = this.memberCount;
        if (allMember < 100) {
          list.push([[0, 99]]);
      ...
      const handler = (members, _, chunk) => {
        ...
        if (members.size < 1_000 || (limit && fetchedMembers.size >= limit) || i === chunk.count) {
          ...
          this.client.removeListener(Constants.Events.GUILD_MEMBERS_CHUNK, handler);
          this.client.removeListener(Constants.Events.GUILD_MEMBER_LIST_UPDATE, handler);
          this.client.decrementMaxListeners();
          let fetched = fetchedMembers.size < this.members.size ? this.members : fetchedMembers;
          if (user_ids && !Array.isArray(user_ids) && fetched.size) fetched = fetched.first();
          if (type == 14) {
            this.client.sleep(sleepTime).then(() => resolve(fetched));
          } else {
            resolve(fetched);
          }
        }
      ...
      this.client.on(Constants.Events.GUILD_MEMBERS_CHUNK, handler);
      this.client.on(Constants.Events.GUILD_MEMBER_LIST_UPDATE, handler);
    });
  }
  fetchBruteforce(options = {}) {
    ...
    if (delay < 500 && !options?.skipWarn) {
      console.warn(
        `[WARNING] Guild#fetchBruteforce: delay is less than 500ms, this may cause rate limits.`,
      );
    }
    return new Promise(async (resolve, reject) => {
      for (const query of dictionary) {
        await this._fetchManyBruteForce({ query, limit }).catch(reject);
  ...
  fetchMemberList(channel, offset = 0, double = false, retryMax = 3, time = 10_000) {
    const channel_ = this.channels.get(channel);
  ...
  edit(data, reason) {
    ...
    if (typeof data.rulesChannel !== 'undefined') {
      _data.rules_channel_id = this.client.resolver.resolveChannelID(data.rulesChannel);
    }
    if (typeof data.publicUpdatesChannel !== 'undefined') {
      _data.public_updates_channel_id = this.client.resolver.resolveChannelID(data.publicUpdatesChannel);
    }
  ...
  /**
   * Marks all messages in this guild as read (v13 alias).
   * @returns {Promise<Guild>}
   */
  markAsRead() {
  ...
  /**
   * Fetch a preview of this guild.
   * @returns {Promise<GuildPreview>}
   */
  fetchPreview() {
    return this.client.fetchGuildPreview(this);
  }

  /**
   * Fetch the widget for this guild.
   * @returns {Promise<Widget>}
   */
  fetchWidget() {
    return this.client.fetchGuildWidget(this);
  }
