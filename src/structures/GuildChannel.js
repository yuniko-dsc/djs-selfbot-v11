...
class GuildChannel extends Channel {
  ...
  memberPermissions(member) {
    ...
    if (member.id === this.guild.ownerID) return new Permissions(member, Permissions.ALL).freeze();
  ...
  overwritePermissions(userOrRole, options, reason) {
    ...
    if (prevOverwrite) {
      payload.allow = BigInt(prevOverwrite.allow);
      payload.deny = BigInt(prevOverwrite.deny);
    }

    for (const perm of Object.keys(options)) {
      if (options[perm] === true) {
        payload.allow |= Permissions.FLAGS[perm] || BigInt(0);
        payload.deny &= ~(Permissions.FLAGS[perm] || BigInt(0));
    ...
    // Convertir en string pour la sérialisation JSON (snekfetch ne supporte pas BigInt)
    payload.allow = payload.allow.toString();
    payload.deny = payload.deny.toString();
