...
class Message {
  ...
  setup(data) { // eslint-disable-line complexity
    ...
    /**
     * The application ID of the application that created this message (if it's an interaction)
     * @type {?Snowflake}
     */
    this.applicationID = data.application_id || null;

    /**
     * The application that created this message (if it's an interaction)
     * @type {?Object}
     */
  ...
  /**
   * Resolve a component by custom ID
   * @param {string} customId The custom ID of the component
   * @returns {?Object} The resolved component
   * @private
   */
  resolveComponent(customId) {
    if (!this.components || !this.components.length) return null;
   ...
   * @example
   * // Click a button
   * message.clickButton('button_id')
   *   .then(() => console.log('Button clicked!'))
   *   .catch(console.error);
   */
  clickButton(customId) {
    const button = this.resolveComponent(customId);
    
    if (!button) {
      throw new TypeError(`Button with custom_id "${customId}" not found`);
    }
    
    if (button.type !== 2 && button.type !== 'BUTTON') {
      throw new TypeError('Component is not a button');
    }
    
    if (button.disabled) {
      throw new TypeError('Button is disabled and cannot be clicked');
    }

    const nonce = require('../util/Snowflake').generate();
    ...
    const data = {
      ...
      message_id: this.id,
      application_id: this.applicationID || this.application?.id || this.author.id,
      session_id: this.client.ws.connection.sessionID,
      message_flags: this.flags ? this.flags.bitfield : 0,
      data: {
        component_type: 2, // BUTTON
        custom_id: button.custom_id || button.customId,
      },
    };

    return this.client.rest.makeRequest('post', '/interactions', true, data);
  }

  /**
   * Forward this message to another channel.
   * @param {ChannelResolvable} channel Target channel
   * @returns {Promise<Message>}
   */
  forward(channel) {
    const targetId = this.client.resolver.resolveChannelID(channel);
    return this.client.rest.methods.forwardMessage(
      targetId,
      this.id,
      this.channel.id,
      this.guild ? this.guild.id : null,
    );
  }

  /**
   * Crosspost this message (announcement channels).
   * @returns {Promise<Message>}
   */
  crosspost() {
    return this.client.rest.methods.crosspostMessage(this.channel.id, this.id).then(() => this);
  }

  /**
   * Fetch the message this message references (reply).
   * @returns {Promise<?Message>}
   */
  fetchReference() {
    if (!this.reference || !this.reference.messageID) return Promise.resolve(null);
    return this.client.rest.methods.getChannelMessage(this.channel, this.reference.messageID);
  }

  /**
   * Re-fetch this message from the API.
   * @returns {Promise<Message>}
   */
  fetch() {
    return this.client.rest.makeRequest(
      'get',
      Constants.Endpoints.Channel(this.channel).Message(this.id),
      true,
    ).then(data => {
      this.setup(data);
      return this;
   ...
   */
  markRead() {
    return this.client.rest.methods.markMessageRead(this.channel.id, this.id).then(() => this);
  }
   ...
   */
  markUnread() {
    return this.client.rest.methods.markMessageUnread(this.channel.id, this.id).then(() => this);
  }
